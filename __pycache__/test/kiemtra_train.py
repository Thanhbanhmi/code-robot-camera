...
def process_frame(frame, seg_point_count=20, regression_point_count=10, conf_threshold=0.7):
    try:
        ...
        for result in results:
            ...
            elif class_id == 2: #   Error
                x1, y1, x2, y2 = box
                error_bounding_boxes.append((x1, y1, x2, y2))
                cv2.putText(annotated_frame, "Error", (x1, y1 -10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)

        return annotated_frame, seg_points, regression_points, regression_equation, error_bounding_boxes
