...
class CameraHandler:
    ...
    def __init__(self, model_path="best.pt"):
        ...
        self.P0 = None
        self.width_video = 3646  # Chiều rộng khung hình
        self.height_video = 2456     # Chiều cao khung hình
        ...
        self.camera_parameters = {
            ...
            2: {  # Webcam
                "scale_x": 300 / 4000,  # Ví dụ
                "scale_y": 200 / 3000,  # Ví dụ
            ...
            3: {  # CSI camera
                "scale_x": 222 / 3646,
                "scale_y": 263 / 2456,
                "px_to_mm_X_Hc_1": 1920 * (222 / 3646),
                "px_to_mm_Y_Hc_1": 1374 * (263 / 2456),
            },
        }
    ...
    def initialize_camera(self, mode, scale=1.0, camera_index=0, log_callback=None):
            ...
            try:
                ...
                if mode == 1:  # Khởi tạo Camera Basler
                    ...
                    print(f"CSI Camera initialized with resolution {1920}x{1080}.")
                    log_callback(f"CSI Camera initialized with resolution {1920}x{1080}.")
                    # Hệ số chống méo của camera
                    self.DIM=(3264, 2464)
                    self.K=np.array([[1440.537083032325, 0.0, 1650.7126495134578], [0.0, 1427.8144289645445, 1292.2500140866257], [0.0, 0.0, 1.0]])
                    self.D=np.array([[0.03610631228367159], [-0.020977866401714916], [0.025216689917902522], [-0.015219021079518105]])
                ...
                try:
                    ...
                    if isinstance(frame, np.ndarray):
                            if self.Mode == 0:  # Chế độ stream
                                ...
                                metal_points = []        # Mảng lưu các điểm tọa độ trên Metal
                                contours = []           # Mảng lưu các giá trị đường biên của mối hàn

                                if results and results[0].masks:  # Kiểm tra kết quả YOLO
                                    ...
                                    for i, mask_data in enumerate(masks):
                                        ...
                                        if class_id == 0:  # Tấm thép
                                             # Lấy 4 điểm góc của bounding box
                                            top_left = (x1, y1)
                                            top_right = (x2, y1)
                                            bottom_left = (x1, y2)
                                            bottom_right = (x2, y2)
                                            metal_points.extend([top_left, top_right, bottom_left, bottom_right])
                                            ...
                                            mask_resized = cv2.resize((mask_data.cpu().numpy() * 255).astype(np.uint8),
                                            ...
                                            for contour in contours:
                                                ...
                                                for point in contour[::self.step_size]:
                                                    cv2.circle(annotated_frame, tuple(point[0]), 10, (0, 255, 255), -1)
                                
#-------------------------------Di chuyển đến tâm tấm thép----------------------------------------------------------------------#
                                camera_params = self.camera_parameters.get(self.mode, {})
                                scale_x = camera_params.get("scale_x", 1)  # Giá trị mặc định là 1 nếu không tìm thấy
                                ...
                                # scale_x = 385 / 3840  # mm/pixel theo chiều X
                                # scale_y = 275 / 2748  # mm/pixel theo chiều Y
                                centroid_x, centroid_y = 0, 0
                                # Trọng tâm tấm thép
                                if len(metal_points) == 4:
                                    # Tính tọa độ trung tâm tấm thép từ 4 điểm góc
                                    centroid_x = int(sum([p[0] for p in metal_points]) / 4)
                                    centroid_y = int(sum([p[1] for p in metal_points]) / 4)
                                                             
                                # #Ma trận đồng nhất Hc1
                                # px_to_mm_X_Hc_1 = 1920 * scale_x    #Trọng tâm của camera theo chiều X
                                # px_to_mm_Y_Hc_1 = 1374 * scale_y    #Trọng tâm của camera theo chiều Y
                                #Ma trận đồng nhất Hc1
                                Hc_1 = np.array([[1, 0, 0, -px_to_mm_X_Hc_1],
                                                [0, 1, 0, -px_to_mm_Y_Hc_1],
                                                [0, 0, 1, 0],
                                                [0, 0, 0, 1]])
                                # Chuyển đổi tọa độ từ pixel sang mm
                                Xc = centroid_x * scale_x
                                Yc = centroid_y * scale_y

                                H1_0 = np.array([[1, 0, 0, 100],    #Toạ độ trung tâm theo trục x
                                                [0, 1, 0, 200],     #Toạ độ trung tâm theo trục y
                                                [0, 0, 1, 0],
                                                [0, 0, 0, 1]])
                                ...
                                self.P0_center_metal = np.dot(Hc_0, Pc0) # Nhân ma trận Hc0 với Pc0  
...
#--------------------------------------------------------------------------------------------------------------------------------------------------------------------------#
                                ...
                                # if cv2.waitKey(10) & 0xFF == ord('q'):  # Nhấn 'q' để thoát
                                #     log_callback("Exiting camera stream...")
                                #     self.running = False  # Kết thúc vòng lặp
                                #     break

                            elif self.Mode == 1:  # Chế độ chụp và xử lý ảnh
                                save_path = "image/captured_image.jpg"
                                with self.lock:
                                    self.current_frame = frame.copy()
                                ...
                                if results and results[0].masks:
                                    ...
                                    boxes = results[0].boxes.xyxy.cpu().numpy()
                                    classes = results[0].boxes.cls.cpu().numpy()
                                    # Mảng lưu các điểm góc của vùng Metal
                                    for i, mask_data in enumerate(masks):
                                        class_id = int(classes[i])
                                        box = boxes[i].astype(int)
                                        x1, y1, x2, y2 = box
                                ...
                                H1_0 = np.array([[1, 0, 0, 100],
                                                [0, 1, 0, 200],
                                                [0, 0, 1, 0],
                                                [0, 0, 0, 1]])
                                Hc_0 = np.dot(Hc_1, H1_0)
#--------------------------------------------Di chuyển biên mối hàn--------------------------------------
                                self.P0_seg_list = []
                                for contour in contours:
                                    for point in contour[::self.step_size]:
                                        seg_x, seg_y = point[0]
                                        ...
                                        Pc0_seg = [[Xc_seg],
                                                ...
                                                [1]]
                                        # Tính tọa độ P0 từ tọa độ Pc0
                                        self.P0_seg = np.dot(Hc_0, Pc0_seg) # Nhân ma trận Hc0 với Pc0    
                                        self.P0_seg = np.ceil(self.P0_seg)  # Làm tròn lên các giá trị
                                        self.P0_seg_list.append(self.P0_seg)   
                                # In kết quả P0
                                for idx, P0_seg in enumerate(self.P0_seg_list):
                                    log_callback(f"P0 for SEG point {idx+1}: {P0_seg.flatten()}")

#-------------------------------------------------------------------------------------------------------
#--------------------------------------------Di chuyển dọc mối hàn--------------------------------------
                                self.P0_interpolated_list = []
                                for point in interpolated_points:                                
                                    ...
                                    Pc0_interpolated = [[Xc_interpolated],
                                                ...
                                                [1]]
                                    # Tính tọa độ P0 từ tọa độ Pc0
                                    self.P0_interpolated = np.dot(Hc_0, Pc0_interpolated) # Nhân ma trận Hc0 với Pc0    
                                    self.P0_interpolated = np.ceil(self.P0_interpolated)  # Làm tròn lên các giá trị
                                    self.P0_interpolated_list.append(self.P0_interpolated)   
                                # In kết quả P0
                                for idx, P0_interpolated in enumerate(self.P0_interpolated_list):
                                    log_callback(f"P0 for Interpolated point {idx+1}: {P0_interpolated.flatten()}")
#-------------------------------------------------------------------------------------------------------
                                ...
                                save_path = f"image_12-02-25/processed_image_{timestamp}.jpg"
